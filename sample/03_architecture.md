...
AST Parserの実体は、`text_compositor/renderer.py`の`TypstRenderer`です。責務ごとに、`renderer_tokens.py`（ブロック要素）・`renderer_inline.py`（インライン要素）・`renderer_tables.py`（表・CSV）・`renderer_layout.py`（レイアウトブロック）・`renderer_diagrams.py`（図表フェンスとSVGのキャッシュ）のミックスインへ分けています。`renderer.py`には、入口（`render_chapter`・`render`）が残っています。
...
4. **ブロック要素（`render_tokens`。`renderer_tokens.py`）**: 見出し・段落・引用・リスト・表・水平線・コードフェンス・HTMLを、トークンの種類ごとにTypstへ変換します。GitHub Alert（`> [!NOTE]`）は`callout`になります。`typst-exec`フェンスは、`reviewed/`ディレクトリの配下にあるファイルだけが使えます。
5. **インライン要素（`render_inline`。`renderer_inline.py`）**: 強調・取り消し線・インラインコード・リンク・画像・文字色・用語集（`[[用語]]`）を変換します。Typstの記号は、ここでエスケープします。
6. **図表フェンス（`renderer_diagrams.py`）**: `mermaid`・`plantuml`・`d2`は、外部ツールでSVGにしてから、画像として埋め込みます。SVGは、種別・ツールのバージョン・コードから決まるキーでキャッシュし、同じ図は描き直しません。`dot`・`graphviz`は、Typst側のテンプレートが描画します。`svg`は、コードそのものが画像です。
