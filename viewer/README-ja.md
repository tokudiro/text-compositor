...
項目は、あと2つあります（#258）。**変換ファイルの保存場所**は、「アプリの領域」（既定）か、「原稿の隣」です。「アプリの領域」は、変換したHTMLと図のキャッシュを、アプリのキャッシュのフォルダに置きます。原稿のフォルダには、何も書かないため、読み取り専用の場所の原稿も、開けます。「原稿の隣」は、原稿の隣の`.text-compositor/`に置きます（ワーカーの既定の動作です）。書き込めない場所の原稿は、「原稿の隣」を選んでいても、アプリの領域に置いて開き、警告を出します。3つ目の「ファイルを作らない」は、まだ実装していないため、灰色で、選べません。**キャッシュ**は、アプリの領域（Windowsでは`%LOCALAPPDATA%\text-compositor\Cache\viewer`）の使用量を表示し、削除のボタンがあります。削除するのは、そこにある、変換したHTMLと図のキャッシュだけで、開いている文書は、変換し直します。原稿の隣の`.text-compositor/`と、取得済みのフォント・図のツールは、消えません。
...
| `src/workdir.js` | 変換したHTMLと図のキャッシュの置き場所（アプリの領域・原稿の隣）と、キャッシュの使用量・削除（#258） |
