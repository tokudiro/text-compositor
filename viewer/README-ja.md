...
| 全画面表示 | `F11`で入り、`Esc`または`F11`で戻ります。全画面では、ツールバーを隠します（エラーの帯は、残ります）。`Ctrl+O`・`F5`・ドラッグ＆ドロップは、全画面でも使えます。 |
...
| `scripts/check-window.js` | 全画面（F11・Esc）と、設定画面を閉じる操作を、実際のキー操作で確認する（手動。Windowsのみ） |
