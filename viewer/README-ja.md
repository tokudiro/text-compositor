...
| 操作 | 方法 |
| --- | --- |
| ファイルを開く | `Ctrl+O`、ウィンドウへのドラッグ＆ドロップ、コマンドラインの引数。起動中に、別のファイルを開くと、既存のウィンドウで表示します。`Ctrl+O`のダイアログは、前回開いたファイルのフォルダから始まり、種類（文・Markdown・図・CSV）で絞り込めます。「文」は、Markdownとテキスト（`.txt`）です。 |
