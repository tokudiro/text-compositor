...
3. **可能な限り、配布ファイルの中に全部入っており、ネットワークへ接続してインストールしなくてよい。**
   ...
   * `pip install`そのものは、導入のためのネットワーク接続を前提とする。
   * **CIは、例外とする。** GitHub Actionsなど、毎回まっさらな、使い捨ての環境（CI）では、実行時に取得してよい。取得物は、`actions/cache`で保存して、取得の回数を減らす（下の「取得の再試行」）。方針（2）（原稿を外へ送らない）は、CIでも保つ。

**現状と方針の差**（2026-09-20時点。実装は、次の表の「現状」のとおりで、方針（3）は、まだ満たしていない。方針の側に、実装を寄せていく。CIでの取得は、例外なので、差に含めない）:

| 対象 | 現状 | 方針との差 |
|---|---|---|
| 日本語フォント（Noto Sans JP、約8.8 MB。PDF） | 初回のビルドで取得する（9章） | 方針（3）: 実行時に取得している |
| Typstのパッケージ（`@preview/...`。`diagraph`など） | 初回のコンパイルで、Typstが取得する | 方針（3）: 同上 |
| PlantUML（JRE 約50 MB＋jar 約17.6 MB） | 初回の描画で取得する（11章） | 方針（3）: 同上 |
...
* **起動時のイベント**: `{"event": "ready", "protocol": 1, "version": "0.3.5"}`。
...
* **対象のファイル**: `.md`・`.markdown`と、図の単体ファイル（`.mmd`・`.puml`・`.d2`・`.dot`・`.gv`。Graphvizは、Viewerでだけ図になる。下記）。それ以外は、エラー。
...
| 図 | Mermaid・PlantUML・D2・`svg`フェンス。Graphviz（`dot`・`graphviz`）は、Viewerのみ（下記） | PDFと同じ仕組みでSVGにし、`<img>`で参照する（Graphvizは、呼び出し元がViz.jsで描く）。`{width= height=}`は、`style`にする。プラグインが無効なら、コードブロックにする（警告なし） |
...
| 未対応（警告） | `typst-exec`、Viewer以外でのGraphviz | 内容を消さずに、コードブロックで表示し、警告する。`typst-exec`は[#182](https://github.com/tokudiro/text-compositor/issues/182)で、HTMLでは実行しない（`reviewed/`の制限は、不要）。Graphvizは、呼び出し元（Viewer）が描画できないとき（CLI・Python API）に、この扱いになる（[#181](https://github.com/tokudiro/text-compositor/issues/181)） |
...
* **Mermaidの描画を、呼び出し元に任せる**（[#207](https://github.com/tokudiro/text-compositor/issues/207)）: 常駐ワーカーは、環境変数`TEXT_COMPOSITOR_MERMAID_HOST=1`で起動されたときだけ、Mermaidの図を、Playwrightとシステムのブラウザではなく、呼び出し元（ViewerのElectron）に描画してもらう。`build`・`render_html`の処理中に、標準出力へ`{"event": "render_mermaid", "callback": N, "diagram_id": ..., "code": ..., "js": "<mermaid.min.jsのパス>"}`を1行出し、標準入力で、`{"callback": N, "ok": true, "svg": "..."}`（失敗は`"ok": false, "error": "..."`）を待つ。待つ間に届いた、番号の違う行は、読み捨てる。呼び出し元が標準入力を閉じたときは、その図の描画の失敗（診断のエラー）になる。`mermaid.min.js`の取得・SHA256の確認・キャッシュは、ワーカー（Python）が行う。描画したSVGは、従来どおり、原稿の隣の`.text-compositor/cache/`にキャッシュされる。プロトコルのバージョンは、1のまま（呼び出し元が、環境変数で、選んだ場合の拡張）。
* **Graphvizの描画も、呼び出し元に任せる**（[#181](https://github.com/tokudiro/text-compositor/issues/181)）: HTML出力のGraphviz（`dot`・`graphviz`のフェンスと、`.dot`・`.gv`のファイル）は、上記と同じ通信で、呼び出し元に描画してもらう。イベントは`render_graphviz`（`{"event": "render_graphviz", "callback": N, "diagram_id": ..., "code": ..., "js": "<viz-global.jsのパス>"}`）で、応答の形式は、Mermaidと同じ。環境変数`TEXT_COMPOSITOR_MERMAID_HOST=1`（互換のため名前は据え置き）が、両方を有効にする。描画できない（環境変数がない。CLI・Python API）ときは、コードブロックで表示し、警告する。`plugins.graphviz: false`のときは、警告なしで、コードブロックにする。描画には、Viz.js（3.30.0。MIT。中に、Graphviz（EPL-2.0）とExpat（MIT）を含む）の`viz-global.js`を使う。Mermaidと同様に、バージョンとSHA256を固定し、初回に取得して、ユーザーのキャッシュ（`viz/`）に置く。同梱はしない。SVGのキャッシュのキーには、Viz.jsのSHA256と、文字幅の補正の版（`GRAPHVIZ_FIT_REVISION`）を含める。PDFのGraphvizは、従来どおり、Typst側の`diagraph`が描き、この経路を通らない（レイアウトの結果が、PDFと少し違う場合がある）。
  * **文字幅の補正**: Graphvizは、文字の幅を、内蔵の見積もり（Times系）で計算する。日本語の長いラベルは、実際より約2割狭く見積もられ、文字が箱からはみ出す（20文字のラベルで確認）。そこで、呼び出し元（Viewer）は、いったん描いたSVGを、Chromium上で、実際のフォントで測り、はみ出したノードだけに`width=`を足して、1回だけ描き直す。記録（`record`）・HTMLラベル・多重の枠のノードは、対象外で、はみ出しが残りうる。補正に失敗したときは、補正なしの図を返す。
  * **`data:`のページでは、動かない**: Viz.jsは、`document.baseURI`から、URLを組み立てる。`data:`のページのままだと、`Invalid URL`でPromiseが拒否される。描画用のページには、形だけの`<base>`を置く（WebAssemblyは、`viz-global.js`に埋め込まれており、そのURLのファイルは、読まれない）。
* **`typst`なしで動く**（[#168](https://github.com/tokudiro/text-compositor/issues/168)）: `typst`（PDF用のコンパイラ）は、Typstをコンパイルするときに、初めて`import`する。HTML出力と、それを使うViewerの配布物は、`typst`と`playwright`を、同梱しない（[viewer-distribution.md](viewer-distribution.md)）。`tests/test_distribution.py`が、この2つを`import`できない状態で、HTML出力が成功することを確認する。`typst`がないままPDFを作ろうとすると、`typst`が要ることを示すエラーになる。
