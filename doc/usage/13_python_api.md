...
- **未対応**: `typst-exec`・生のHTMLは、内容をコードブロックで表示して、警告します。Graphviz（`dot`・`graphviz`）は、PDFと同じく、Typstの`diagraph`で図になります（`typst`が要ります）。`shape=record`・図全体の`label`は、描けないため、警告します。数式と、複数ファイルの出力は、ありません。
...
```json
{"event": "ready", "protocol": 1, "version": "0.3.8"}
```
