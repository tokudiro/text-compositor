...
`graphviz`/`pikchr`/`cetz`/`fletcher`/`mermaid`/`plantuml`/`d2`を`false`にすると、該当する図表フェンス（`dot`/`graphviz`/`pikchr`/`cetz`/`fletcher`/`mermaid`/`plantuml`/`d2`言語のコードブロック）は描画せず、素のコード表示にフォールバックします。`mermaid: true`の場合は`playwright`パッケージが、`plantuml: true`の場合はローカルのJava（11以上）が、`d2: true`の場合はD2 CLI本体が必要です。
